...
local function run()
  ...
  local timeout_set, timeout_set_err = server:set_timeout(0.02)
  if not timeout_set then
    server:close()
    client:close()
    listener:close()
    return nil, timeout_set_err
  end
  ...
  local _, closed_read_err = server:read(1)
  if not closed_read_err then
    server:close()
    listener:close()
    return nil, "expected closed read failure"
  end
  if closed_read_err.kind ~= "closed" then
    server:close()
    listener:close()
...
return {
  name = "tcp luv listen/dial loopback",
  run = run,
}

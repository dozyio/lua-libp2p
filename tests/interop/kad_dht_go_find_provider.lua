...
if not bootstrap_addr or bootstrap_addr == "" or not key_hex or key_hex == "" then
  io.stderr:write("usage: lua tests/interop/kad_dht_go_find_provider.lua <bootstrap-multiaddr> <key-hex>\n")
  os.exit(2)
end
...
local op, op_err = host.kad_dht:find_providers(key, {
  limit = 1,
  alpha = 1,
  disjoint_paths = 1,
})
...
if #(result.providers or {}) < 1 then
  io.stderr:write("expected at least one provider\n")
  os.exit(1)
end

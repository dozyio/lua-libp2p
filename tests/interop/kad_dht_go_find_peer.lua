...
local host, host_err = host_mod.new({
  ...
  peer_discovery = {
    bootstrap = {
      ...
      config = { list = { bootstrap_addr }, dialable_only = true, ignore_resolve_errors = true, dial_on_start = true },
    },
  },
...
local op, op_err = host.kad_dht:find_peer(
  peer_id,
  { use_cache = false, peers = { { peer_id = peer_id, addrs = { bootstrap_addr } } }, alpha = 1, disjoint_paths = 1 }
)
if not op then

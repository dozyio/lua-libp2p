...
local function run()
  ...
  if not ok_pkey or not ok_digest then
    return nil, "luaossl is required for rsa noise test"
  end

  local private_key, key_err = pkey.new({ type = "RSA", bits = 2048 })
  ...
  if not public_der or public_der == "" then
    return nil, "failed to export generated rsa public key"
  end
